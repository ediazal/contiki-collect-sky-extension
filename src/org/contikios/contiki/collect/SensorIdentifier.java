package org.contikios.contiki.collect;

public interface SensorIdentifier {
  public static final String CO_SENSOR = "CO";
  public static final String CO2_SENSOR = "CO2";
  public static final String DUST_SENSOR = "Dust";
  public static final String LIGHT1_SENSOR = "Light1";
  public static final String LIGHT2_SENSOR = "Light2";
  public static final String TEMPERATURE_SENSOR = "Temperature";
  public static final String HUMIDITY_SENSOR = "Humidity";
}
